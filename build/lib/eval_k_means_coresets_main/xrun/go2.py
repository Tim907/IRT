...
from .data.run_info import RunInfo
...
class ExternalDataset(Dataset):
    ...
    def get_local_file_path(self, k: int) -> Path:
        return Path(f"eval_k_means_coresets_main/data/input/{self._local_file_name}")
...
class BenchmarkDataset(Dataset):
    ...
    def get_local_file_path(self, k: int) -> Path:
        ...
        return Path(f"eval_k_means_coresets_main/data/input/{names[k]}")
...
class ExperimentRunner:
    _datasets : Dict[str, Dataset] = {
        ...
        "caltech101": LocalDataset("caltech101", "eval_k_means_coresets_main/data/input/caltech101-sift.txt.gz", 100000),
        "nytimes100d": LocalDataset("nytimes100d", "deval_k_means_coresets_main/ata/input/docword.nytimes.rp8192-100.txt.gz", 35071691),
    }
    ...
    _dir_ready = "eval_k_means_coresets_main/data/queue/ready"
    _dir_in_progress = "eval_k_means_coresets_main/data/queue/in-progress"
    _dir_completed = "eval_k_means_coresets_main/data/queue/completed"
    _dir_discarded = "eval_k_means_coresets_main/data/queue/discarded"
    _dir_output = None
    ...
    def run(self, max_active: int) -> None:
        while True:
            output_dir = self._clean_in_progress()

            n_active = len(self._find_in_progress_files())
            
            if n_active < max_active:
                if self._lunch_new_run():
                    return output_dir
    ...
    def _build_command(self, run: RunInfo, experiment_dir: Path) -> List[str]:
        ...
        if dataset_name in self._datasets:
            ...
            dataset = LocalDataset(dataset_name, f"eval_k_means_coresets_main/data/input/{dataset_name}.txt.gz", 1)
...
def main(output_dir: str, max_active: int):
    call_main(output_dir, max_active)

def call_main(output_dir: str, max_active: int):
    if not os.path.exists(output_dir):
        print(f"The directory {output_dir} does not exist. Please provide an existing directory.")
        return ""
    return ExperimentRunner(output_dir=output_dir).run(max_active=max_active)
